...
public class SimpleIKSolverEditorScript : Editor
{
    ...
    static void OnDrawGizmosSelected(SimpleIKSolver siks, GizmoType gizmoType)
    {
        ...
        Handles.DrawDottedLine(siks.tip.position, siks.target.position, 3);
        ...
        Handles.Label(siks.target.position, "Target");
        var distanceToTarget = Vector3.Distance(siks.target.position, siks.tip.position);
        var midPoint = Vector3.Lerp(siks.target.position, siks.tip.position, 0.5f);
    ...
    public void OnSceneGUI()
    {
        ...
        siks.target.position = Handles.PositionHandle(siks.target.position, Quaternion.identity);
